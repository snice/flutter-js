<route>
{"title": "fetch", "desc": "dog.ceo + httpbin 实测 fetch 全链路", "group": "网络请求"}
</route>
...
// httpbin.org 自己经常 503（Cloudflare），httpbingo 是它的可靠替代——
// 差异只有一处：headers 的值是数组
const BIN = 'https://httpbingo.org';
...
const cases: Case[] = [
  ...
  },
  {
    ...
    run: async () => {
      ...
      const body = (await res.json()) as { headers: Record<string, unknown> };
      const raw = body.headers['X-Fjs-Test'];
      const sent = Array.isArray(raw) ? String(raw[0]) : String(raw);
