...
| `getContext('webgl')` / `'webgl2'` | ⚠️ | **模块提供**（spec 021 实现、022 抽出）：`npm i @ufjs/webgl` 并 `import '@ufjs/webgl'` 才有；不装则两端一致返回 `null` + 一条告警。装后 GL 指令编码进 op 11，App 侧经 [flutter_angle](https://pub.dev/packages/flutter_angle)（ANGLE → Metal/Vulkan）执行，web 侧透传浏览器原生 context。坐标是**位图像素**（逻辑 × dpr，`gl.canvas.width/height`），页面自己处理 dpr——这是与 2d 刻意的差异，见 §11。`getExtension()` 返回 `null`；`readPixels` ❌（整图导出走 `toDataURL`）；一张画布只能有一种 context 类型（DOM 语义，先到先得）。WebGL2 的 VAO 与 `texStorage2D` / `texSubImage2D(source)` 双端可用（spec 023，three.js 的上传路径）；instancing（`vertexAttribDivisor` + `drawArraysInstanced` / `drawElementsInstanced`，three 的 `InstancedMesh`）双端可用（spec 033），WebGL1 的 `ANGLE_instanced_arrays` 扩展不提供。**每张画布的 context 状态独立**（spec 036）：flutter_angle 在各平台只有**一个** GL context，画布只是挂在上面的 surface，App 侧宿主因此按画布记账 enable/blend/depth/stencil、viewport/scissor、各类绑定与通用顶点属性值，切到某张画布执行或查询前只把与 GL 当前值不同的项补发一遍；默认 VAO 由每张画布一个隐藏 VAO 充当，顶点属性状态天然隔离。修之前的现象：先开一个 `enable(DEPTH_TEST)` 的页面，再开一个不碰深度的页面，后者只剩清屏色。命令流没带的 WebGL2 方法在 App 侧是**不存在**（调用即 `not a function`），不是告警。`getShaderPrecisionFormat` 在 App 侧按 WebGL2 下限常量作答（flutter_angle 的实现是全零 stub，照抄会让 three 选 lowp）。**呈现是每帧一次的**（spec 028）：GL 命令的**执行**跟着 chunk 到达走（页面在 `draw()` 中途做 GL 查询会把命令流冲一次，一个页面帧因此分两批到达宿主），但**呈现**每个 Flutter 帧至多一次，且必须等这个节点的 `Texture` layer 建出来。`updateTexture` 两端都是 `eglSwapBuffers`，EGL 默认 `EGL_BUFFER_DESTROYED`——一帧 swap 两次，第二次推的就是已被丢弃的缓冲；一帧跨两个缓冲，清屏和几何会落在不同缓冲上。按需渲染的页面（只画一帧）会把这两种错误状态一直挂在屏幕上，连续渲染的页面则被下一帧盖过去，所以这类 bug 只在按需渲染的页面上现形。**iOS 真机**：flutter_angle 走 IOSurface→EGL pbuffer 路径（模拟器走 Metal 纹理/FBO 路径，机制不同），swap 前需要一次 1×1 `glReadPixels` 做真同步——`eglSwapBuffers` 不等 GPU，而 ANGLE 的 Apple 后端上 **`glFinish` 也不等**（spec 026 曾把功劳记给 `glFinish`，028 在 swap 前读像素证伪：画面全空时中心已是模型色），读回则必须等命令队列排空。这一条只在 Apple 的设备 surface 路径生效，模拟器与 Android 不付这个流水线停顿。EGL surface 建不出来时画布会以一条 `[fjs] webgl:` 日志显性失败而非静默空白|
...
首帧真的贵的才开。仓库里开了的是 `/example/canvas/f2` 和三个 webgl / three.js 页。
