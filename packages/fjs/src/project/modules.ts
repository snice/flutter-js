...
/** The full content of the host's generated `lib/fjs_autolink.dart`.
 *
 * The register calls used to be inlined in main(), which made main.dart the
 * one file whose content changed whenever a module was added — and also the
 * one file users want to hand-edit (an extra host.register, a Flutter plugin
 * init). Keeping the volatility in this generated module is what lets
 * main.dart be static and written only when missing: adding or removing a
 * module now touches only this file. */
export function autolinkDartModule(entries: AutolinkEntry[]): string {
  const { imports, registers } = autolinkDart(entries);
  const body =
    registers || '  // no modules with a Flutter side are linked into this host\n';
  return `// generated by fjs — do not edit.
//
// Autolinked modules: their imports and register() calls, rewritten on every
// run. main.dart calls fjsRegisterModules(engine) once, before runApp.
