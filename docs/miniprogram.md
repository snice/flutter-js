...
| sticky-header / sticky-section | **按渲染器分流**（specs/052）。**skyline**：原生组件原样直出，`offset-top` / `push-pinned-header` / `bindstickontopchange` 都交给 skyline；必须是 `type="custom"` scroll-view 的直接子节点。注意 skyline 原生组件在首帧会对每个 header 先派一次 `isStickOnTop:false`（web / Flutter 端首帧静默），页面若以「收到事件」为吸顶依据需自行忽略首次。**webview**：原生组件不存在，降级为 `view` + `fjs-sticky-header` / `fjs-sticky-section`（wxss `position: sticky`，吸顶边界 = 父元素盒子，与 web 同一 substrate）；静态数字 `offset-top` 映射成内联 `top`，绑定的 `offset-top` 告警后不生效，`allow-overlapping` / `padding` / `push-pinned-header` 接受但不生效 |
