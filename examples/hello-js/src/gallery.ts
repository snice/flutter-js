// 组件总览：底层 element API 的用法示例，不经过任何框架。
//
// 这一屏原本就是 hello-js 的全部内容。现在它和主题压测屏共用一个 app，所以
// 它被包成一个可挂载 / 可卸载的函数：压测屏在量帧的时候，这一屏的 setInterval
// 和 Worker 必须是停着的——每 400ms 一次 setText 就是一帧，采样窗口里混进来
// 的每一帧都会污染「最慢帧」。
import { h, insert, setText, toast, Worker, type Element } from 'fjs';

export function mountGallery(host: Element): () => void {
  // scroll-view 而不是 view：这一屏比一屏高，而且它现在挂在 tab 壳里，
  // 没人替它撑满或者滚动
  const root = h('scroll-view', { style: { flexGrow: 1 } });
  insert(host, root);
